...
def create_horizontal_bar_chart(df, institution_name, yearly_data):
    ...
    if yearly_data is None or yearly_data.empty:
      return None # Handle the case where there's no yearly data
    
    year_columns = [col for col in yearly_data.columns if isinstance(col, str) and col.endswith('년')]
    
    # Ensure that the yearly data actually contains these columns
    if not year_columns:
        return None
...
def create_yearly_revenue_pie_charts(df, institution_name, yearly_data):
    ...
    for year in year_columns:
        ...
        yearly_data_with_type['매출액'] = yearly_data_with_type[[year]].sum(axis=1) # Calculate revenue for current year
        
        yearly_sums_by_type = yearly_data_with_type.groupby('훈련유형')['매출액'].sum().reset_index()
        yearly_sums_by_type['매출액'] = yearly_sums_by_type['매출액'].apply(lambda x: x / 100000000) # Convert to 억
...
def analyze_training_institution(df, yearly_data):
    ...
    if selected_institution:
        ...
        course_summary['매출액'] = course_summary['매출액'].apply(lambda x: format_revenue(x)) # Convert to 억
...
def analyze_course(df, yearly_data):
    st.subheader("과정 분석")
    # 추가적으로 과정 분석 관련 로직을 구현할 수 있습니다.
    st.write("과정 분석 기능은 아직 구현 중입니다.")

def analyze_ncs(df, yearly_data):
    st.subheader("NCS 분석")
    # 추가적으로 NCS 분석 관련 로직을 구현할 수 있습니다.
