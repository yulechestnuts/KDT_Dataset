...
def create_training_type_analysis(df):
    st.header("K-디지털 훈련유형별 분석")
    
    # 훈련유형별 통계
    type_stats = df.groupby('훈련유형').agg({
        '누적매출': 'sum',
        '과정명': 'count'
    }).reset_index()
    
    # 전체 매출 대비 비율 계산
    total_revenue = type_stats['누적매출'].sum()
    type_stats['비율'] = type_stats['누적매출'] / total_revenue * 100
    
    # 시각화
    fig = px.pie(type_stats, 
                 values='누적매출', 
                 names='훈련유형',
                 title='훈련유형별 매출 비중')
    st.plotly_chart(fig)
    ...
    # 상세 통계
    st.subheader("훈련유형별 상세 통계")
    for _, row in type_stats.iterrows():
        with st.expander(f"{row['훈련유형']} 상세정보"):
            col1, col2 = st.columns(2)
            with col1:
                st.metric("총 매출", f"{row['누적매출']/100000000:.1f}억원")
            with col2:
                st.metric("과정 수", f"{row['과정명']}개")
            st.write(f"전체 매출 대비 비중: {row['비율']:.1f}%")
